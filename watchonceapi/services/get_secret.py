...
def get_file_download_url(base_url: str,
                          port: Optional[int],
                          filepath: str):
    if port:
...
@inject
def get_files_links(filepaths: List[str],
                    client: Minio = Provide[Container.minio_client]) \
        -> List[str]:
    files = []
    for filepath in filepaths:
        url: str = client.get_presigned_url("GET",
...
async def get_secret_or_404(uuid_: str,
    ...
    return ResponseSecretSchema(text=secret[0],
                                files=files,
                                expires_at=secret[1])
